...
validate_guc_settings() {
  ...
  if ! (su - "${USER}" -c "gpstate") &> /dev/null; then
  ...
  gp_resource_group_memory_limit_x100=$(su - "${USER}" -c 'gpconfig -s gp_resource_group_memory_limit' | grep "^Master" | awk '{ printf $3 * 100 }')

  num_active_primary_segments=$(su - "${USER}" -c "psql -d postgres -t -c \"select count(1) from gp_segment_configuration where content <> -1 and preferred_role = 'p'\"" | awk '{ printf $1 }')
  ...
  max_expected_concurrent_queries=$(su - "${USER}" -c "psql -d postgres -t -c \"SELECT concurrency FROM gp_toolkit.gp_resgroup_config where groupname = 'default_group'\"")
  ...
  max_acting_primary_segments=$(su - "${USER}" -c "psql -d postgres -t -c \
    \"with hostnames as ( \
  ...
  cmd_val_contains "su - ${USER} -c 'gpconfig -s gp_interconnect_queue_depth'" "Master  value: 16"
  cmd_val_contains "su - ${USER} -c 'gpconfig -s gp_interconnect_queue_depth'" "Segment value: 16"

  cmd_val_contains "su - ${USER} -c 'gpconfig -s gp_interconnect_snd_queue_depth'" "Master  value: 16"
  cmd_val_contains "su - ${USER} -c 'gpconfig -s gp_interconnect_snd_queue_depth'" "Segment value: 16"

  cmd_val_contains "su - ${USER} -c 'gpconfig -s gp_resource_group_memory_limit'" "Master  value: 0.85"
  cmd_val_contains "su - ${USER} -c 'gpconfig -s gp_resource_group_memory_limit'" "Segment value: 0.85"

  cmd_val_contains "su - ${USER} -c 'gpconfig -s statement_mem'" "Master  value: ${statement_mem_with_unit}"
  cmd_val_contains "su - ${USER} -c 'gpconfig -s statement_mem'" "Segment value: ${statement_mem_with_unit}"

  cmd_val_contains "su - ${USER} -c 'gpconfig -s max_statement_mem'" "Master  value: ${max_statement_mem_with_unit}"
  cmd_val_contains "su - ${USER} -c 'gpconfig -s max_statement_mem'" "Segment value: ${max_statement_mem_with_unit}"

  cmd_val_contains "su - ${USER} -c 'gpconfig -s gp_vmem_protect_limit'" "Master  value: ${gp_vmem_protect_limit}"
  cmd_val_contains "su - ${USER} -c 'gpconfig -s gp_vmem_protect_limit'" "Segment value: ${gp_vmem_protect_limit}"

  cmd_val_contains "su - ${USER} -c 'gpconfig -s gp_workfile_compression'" "Master  value: off"
  cmd_val_contains "su - ${USER} -c 'gpconfig -s gp_workfile_compression'" "Segment value: off"
}
